...
VulkanRenderer::VulkanRenderer(): /// Initialize all the variables
    window(nullptr), instance(nullptr), debugMessenger(0), surface(0),commandPool(0),
...
void VulkanRenderer::initVulkan() {
    ...
    pickPhysicalDevice();
    // Collect data about hardware
    // Make logical devices out of physical ones and collect data about hardware
    createLogicalDevice();
    // Enable double buffering
    // Collect data, set up and create swap chains
    createSwapChain();
    createImageViews();
    // Collect data, set up and create a render pass
    // Render pass is an aboject that stores data about images
    createRenderPass();
    // Set up structures
    ...
    createCommandPool();
    // It is about depth buffer and how to set up info about this buffer
...
void VulkanRenderer::pickPhysicalDevice() {
    ...
    // Checks if devices are suitable
...
void VulkanRenderer::createLogicalDevice() {
    ...
    // setting up the queues (data of my hardware)
    ...
    // Collects data about device features
...
void VulkanRenderer::createSwapChain() {
    ...
    // Double buffering - the memory is divided into two chunks, so that 
    // one chunk has an old image and the other processes a new image. 
    // When the new image is done the memory swaps.
    // Thus, the image doesnt flicker, while loading

    // What does you hardware support
...
/*void VulkanRenderer::createDescriptorSetLayout() {
    ...
    if (vkCreateDescriptorSetLayout(device, &layoutInfo, nullptr, &shaderComponent->descriptorSetLayout) != VK_SUCCESS) {
...
void VulkanRenderer::createGraphicsPipeline(const char* vFilename, const char * fFilename) {
    ...
    pipelineLayoutInfo.pSetLayouts = &shaderComponent->descriptorSetLayout;
    pipelineLayoutInfo.pPushConstantRanges = &push_constant;
    pipelineLayoutInfo.pushConstantRangeCount = 1;  

    if (vkCreatePipelineLayout(device, &pipelineLayoutInfo, nullptr, &shaderComponent->pipelineLayout) != VK_SUCCESS) {
    ...
    pipelineInfo.layout = shaderComponent->pipelineLayout;
    pipelineInfo.renderPass = renderPass;
    pipelineInfo.subpass = 0;
    pipelineInfo.basePipelineHandle = VK_NULL_HANDLE;

    if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &shaderComponent->graphicsPipeline) != VK_SUCCESS) {
...
void VulkanRenderer::createUniformBuffers(VkDeviceSize bufferSize, 
...
void VulkanRenderer::createDescriptorPool(Descriptor& descriptor) {
...
void VulkanRenderer::createDescriptorSets(Ref<MaterialComponent> materialComponent_, Descriptor& descriptor) {
    ...
    for (size_t i = 0; i < swapChainImages.size(); i++) {
        ...
        glightsBufferInfo.buffer = glightingBuffers[i];
...
void VulkanRenderer::recordCommandBuffer() {
    for (size_t i = 0; i < commandBuffers.size(); i++) {
        ...
        for (int j = 0; j < std::size(pushConst); j++) {
            ...
            vkCmdBindVertexBuffers(commandBuffers[i], 0, 1, &vertexBuffer[j].bufferID, offsets);

            vkCmdBindIndexBuffer(commandBuffers[i], indexBuffer[j].bufferID, 0, VK_INDEX_TYPE_UINT32);

            vkCmdBindDescriptorSets(commandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, shaderComponent->pipelineLayout, 0, 1, &descriptor[j].sets[i], 0, nullptr);

            vkCmdDrawIndexed(commandBuffers[i], static_cast<uint32_t>(modelParameters[j].indices.size()), 1, 0, 0, 0);
        }
...
void VulkanRenderer::updateUniformBuffer(uint32_t currentImage) {
   ...
   /// UniformBufferObject ubo{}; Gone, now  its a member of the class
    ...
    void* data;
    // Grab an address from the Graphic Card
    // We are asking it to go down to a memory card and 
    // find a block of memory large enought to hold ubo
    vkMapMemory(device, cameraBuffersMemory[currentImage], 0, sizeof(UniformBufferObject), 0, &data);
...
{
    void* data;
    vkMapMemory(device, glightingBuffersMemory[currentImage], 0, sizeof(GlobalLighting), 0, &data);
    memcpy(data, &glightsUBO, sizeof(GlobalLighting));

    vkUnmapMemory(device, glightingBuffersMemory[currentImage]);
}
